...
- [ ] `LICENSE` at plugin root: DarkOffice does not require it for local plugins, but it is **required** at the repo root before submitting to the Plugin Index. If missing → **WARN** — `LICENSE absent — required for community contribution (Plugin Index); optional for local-only use`
...
Read source files and check for violations of DarkOffice conventions.
