# Development manual for DarkOffice
This guide will show you how to setup a local development environment for DarkOffice in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - DarkOffice runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- DarkOffice is available on GitHub [github.com/agent0ai/agent-zero](https://github.com/agent0ai/agent-zero).
...
- DarkOffice comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.
...
2. Prepare the python environment to run DarkOffice in. This step assumes you have some Python runtime installed. By clicking the python version in lower right corner (3.13.1 in my example), you should get a list of available environments. You can click the `+ Create Virtual Environment` button. You might be prompted to select the environment manager if you have multiple installed. I have venv and Conda, I will select Conda here. I'm also prompted for desired python version, I will select 3.12, that is known to work well.
...
## Step 4: Run DarkOffice in the IDE
Great work! Now you should be able to run DarkOffice from your IDE including real-time debugging.
...
After inserting my API key in settings, my DarkOffice instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of DarkOffice in Docker
...
DarkOffice runs code inside the container by default. If you are running the framework locally in your IDE but want tools (like code execution) to run in Docker, configure RFC in **Settings -> Development** and point it to a running DarkOffice container. This routes execution through SSH/RFC to the container while keeping the UI and agent loop on your host.


# Congratulations!

You have successfully set up a complete DarkOffice development environment! You now have:
...
- The ability to develop, debug, and test DarkOffice features seamlessly

You're now ready to contribute to DarkOffice, create custom extensions, or modify the framework to suit your needs. Happy coding!
...
- See [Contributing to DarkOffice](../guides/contribution.md) for contribution basics.
