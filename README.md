...
Based on the open-source Agent Zero framework. Upstream links and technical identifiers are retained for compatibility.

### Give your agent a full Linux computer.

DarkOffice is an open agent framework for work that needs more than chat: a Dockerized Linux desktop, a browser with DOM annotation, live document cowork, projects, skills, plugins, and a bridge back to your host machine.
...
<div align="center">
<img alt="DarkOffice driving Blender in its built-in XFCE desktop" src="docs/res/usage/webui/agentzero-xfce-computer.gif" width="100%" />
</div>
...
The desktop **A0 Launcher** is the fastest guided path on a personal machine. Download it, open it, and let it check Docker, create Instances, manage ports, and connect to local or remote DarkOffice installs.

DarkOffice runs wherever Docker runs, from a $6 VPS or Raspberry Pi to a local workstation or GPU server.
...
DarkOffice opens its own Linux desktop inside the right-side Canvas. Not a remote VM, not a shared clipboard, but a real XFCE desktop session running in the container.
...
<img alt="Annotating a webpage element in the DarkOffice browser" src="docs/res/usage/browser/annotation.gif" />
<br>

DarkOffice ships a built-in Browser with an optional live surface in the Canvas. The agent can open pages, read them, click, type, upload files, and take screenshots - the usual. The unusual part is **Annotate mode**.
...
<img alt="DarkOffice writing a TODO plan in the Canvas markdown editor" src="docs/res/usage/webui/markdown-editor.gif" />
<br>
...
LibreOffice Writer, Calc, and Impress are wired up so you can type by hand while DarkOffice creates, updates, saves, and verifies the same files in real time.

ODT, ODS, and ODP binary formats are first-class citizens in the DarkOffice Desktop environment to align with the Open Document Format (ODF).
...
<img alt="DarkOffice Plugin Hub showing community plugins" src="docs/res/usage/plugins/plugin-hub-browse.png" />
<br>

DarkOffice is built for extension, not just configuration. The built-in **Plugin Hub** browses a growing catalog of community plugins - currently more than 100, covering:
...
Install with a click from the Web UI, or publish your own to the index repository. Combined with custom prompts in `prompts/`, custom tools in `tools/`, MCP servers, A2A connectors, and project-scoped configuration, DarkOffice gives you a real surface area to shape the agent into whatever you need.
...
<img alt="OAuth LLM plans in DarkOffice" src="docs/res/codex-screenshot.png" />
<br>

DarkOffice connects to your OpenAI Codex plan through the new OAuth flow. Sign in with your account, pick the Codex-backed provider, and let DarkOffice use the plan you already have. Click "Connect", enter the device code in the OpenAI page, choose your model, and you're set.

This is the first step toward account-backed LLM plans in DarkOffice. More integrations are coming, including Gemini CLI and Claude Code through extra-usage.
...
The **A0 CLI Connector** is not a separate CLI agent. It connects to a running DarkOffice instance and gives that instance a terminal-native bridge to your host machine - so the same agent (with all its memory, projects, and skills) can also work on real files outside the Docker container.

Install the connector on the machine you want DarkOffice to work on, **not** inside the DarkOffice container.
...
Then run `a0` to connect your terminal to an existing DarkOffice instance. It can usually discover a local instance automatically, or you can point it at a remote URL hosted somewhere else, such as a VPS or tunnel.

This is especially useful if you:

- prefer CLI workflows;
- want DarkOffice to work in an existing local repository;
- are running DarkOffice on a remote server;
- want Docker isolation for DarkOffice while still granting explicit, controlled access to host-side work.
...
**Skills** can be loaded on demand by DarkOffice, or pinned from the chat input when you want a specific procedure to stay active.
...
This makes DarkOffice useful for research, software engineering, data analysis, plugin development, and tasks where several specialized perspectives are better than one overloaded context.
...
DarkOffice supports plugins, MCP, A2A, custom tools, custom prompts, project-scoped configuration, environment-based deployment settings, and a Web UI designed to keep the agent's work readable in real time.

## Time Travel

Time Travel gives DarkOffice-owned `/a0/usr` workspaces snapshot history, diff inspection, travel, and revert. It is designed for recoverable agent work: see what changed, compare files, inspect a past state, and roll back when needed.
...
- **Host-machine development:** connect with `a0` and let DarkOffice work in your real local repositories, or clone them through Git Projects feature in the Web UI.
...
| --- | --- |
| Install or update DarkOffice | [Installation](./docs/setup/installation.md) |
...
| Connect DarkOffice to host-machine files and shell | [A0 CLI Connector](https://www.agent-zero.ai/p/docs/a0-cli-connector/) |
...
| Understand the architecture and internals | [DeepWiki for upstream Agent Zero](https://deepwiki.com/agent0ai/agent-zero) |
...
DarkOffice is built for people who want to understand and shape their tools.
...
[Space Agent](https://github.com/agent0ai/space-agent) is the related, more polished product direction for the agent-shaped workspace. DarkOffice remains the open framework and Linux-powered workbench.

## Safety Model

DarkOffice is powerful because it can use a real environment.
