...
- Own DarkOffice's LiteLLM transport adapter for Chat Completions and Responses API calls.
- Normalize DarkOffice model-call kwargs into provider-safe LiteLLM requests.
...
- Strip DarkOffice internal kwargs before sending requests to LiteLLM.
...
- When DarkOffice function tools are present, default Responses requests to one required native call; explicit request-level `tool_choice` and `parallel_tool_calls` values still win.
