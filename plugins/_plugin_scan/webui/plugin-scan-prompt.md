...
- Treat expected filesystem access as {{RATING_PASS}} when it is limited to plugin-owned paths, configuration, cache/state files, explicit user-selected files, DarkOffice workdir/project resources needed by the feature, or temporary directories with cleanup.
