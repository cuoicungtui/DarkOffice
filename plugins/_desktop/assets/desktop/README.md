...
This Desktop is DarkOffice's habitat: a visible, stateful Linux workspace where humans and agents can share files, launch tools, inspect results, and turn reasoning into observable work.
...
- Jan Tomášek, for creating DarkOffice and making this whole workshop possible.

DarkOffice stands on this craft with respect. Open source is not just code we consume; it is shared affordance, accumulated care, and an invitation to build systems that are powerful without becoming opaque.

## Open the Terminal

Double-click **Terminal** on this Desktop. You will land in the DarkOffice workdir, where agent CLIs can inspect files, edit projects, and run commands inside the Linux environment.
...
If in doubt, start with Codex or Claude Code for coding, Aider for a lighter patch-focused loop, Goose or OpenHands for a broader local agent, and Hermes or OpenClaw when you want the agent to grow into more of a persistent runtime working together with DarkOffice.
