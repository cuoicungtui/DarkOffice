...
const DESKTOP_START_MESSAGE = "Starting DarkOffice Desktop environment";
const DESKTOP_RUNTIME_INSTALL_MESSAGE = "Installing DarkOffice Desktop runtime dependencies. This can take a few minutes after an update.";
...
const model = {
  ...
  async restartDesktopSession() {
    ...
    const session = await this.ensureDesktopSession({
      ...
      restart: true,
      select: true,
      message: "Restarting DarkOffice Desktop environment",
    });
    if (!session) {
  ...
  async openDesktopWhenRuntimeReady(showProgress = true) {
    ...
    while (response?.ok === false && this.isDesktopRuntimeInstalling(response)) {
      ...
      if (Date.now() - startedAt > DESKTOP_RUNTIME_INSTALL_TIMEOUT_MS) {
        return {
          ...response,
          error: "DarkOffice Desktop runtime installation is still running. Please try again in a moment.",
        };
      }
