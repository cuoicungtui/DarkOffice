...
function mount() {
  ...
  const load = async () => {
    const project = currentAgentProject(); const requestedStrategy = model.strategyId; model.agentProjectName = project; const requestId = ++model.requestId;
    if (!project) { model.data = null; projectContext.textContent = "Chưa chọn dự án Agent"; content.innerHTML = "<p class='empty'>Hãy chọn một Dự án Agent để xem dữ liệu.</p>"; return; }
    content.innerHTML = "<p class='empty'>Đang tải...</p>";
    try { const contextResponse = await callJsonApi("/api/strategy",{action:"get_project_context",agent_project_name:project}); const strategyResponse = await callJsonApi("/api/strategy",{action:"list_strategies",agent_project_name:project}); if (requestId !== model.requestId) return; const context = contextResponse.context || {}; model.strategyId = requestedStrategy || context.active_strategy?.id || null; model.strategies = strategyResponse.strategies || []; strategySelect.innerHTML = model.strategies.map((item) => `<option value="${esc(item.id)}">${esc(item.name)} · ${esc(item.status)}</option>`).join("") || `<option value="">Chưa có chiến lược</option>`; strategySelect.value = model.strategyId || ""; projectSelect.innerHTML = `<option value="">Tất cả dự án</option>${(context.plane_projects || []).map((item) => `<option value="${esc(item.remote_id)}">${esc(item.title)}</option>`).join("")}`; projectContext.textContent = `Dự án Agent: ${project} · Plane Workspace: ${context.plane_workspace?.plane_workspace_slug || "chưa kết nối"} · Chiến lược: ${model.strategies.find((item) => item.id === model.strategyId)?.name || context.active_strategy?.name || "chưa có"}`; const response = await callJsonApi("/api/strategy",{action:"dashboard",agent_project_name:project,filters:{agent_project_name:project,strategy_id:model.strategyId}}); if (requestId !== model.requestId) return; model.data = response.data; draw(); }
