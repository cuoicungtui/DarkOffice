...
function leafWorkItems(items) {
  const parentIds = new Set(items.map(workParent).filter(Boolean));
  return items.filter((item) => !parentIds.has(item.remote_id));
}
function isComplete(item) { return ["completed", "done"].includes(String(item.state_group || "").toLowerCase()); }
function isCancelled(item) { return ["cancelled", "canceled"].includes(String(item.state_group || "").toLowerCase()); }
function targetDate(item) {
  const value = item.target_date || json(item).target_date;
  if (!value) return null;
  const date = new Date(value);
  return Number.isNaN(date.valueOf()) ? null : date;
}
function dateText(item) {
  const date = targetDate(item);
  if (!date) return "Chưa đặt hạn";
  const day = date.toLocaleDateString("vi-VN");
  if (!isComplete(item) && !isCancelled(item) && date.getTime() < Date.now()) return `Quá hạn từ ${day}`;
  return `Hạn ${day}`;
}
function isOverdue(item) {
  const date = targetDate(item);
  return Boolean(date && date.getTime() < Date.now() && !isComplete(item) && !isCancelled(item));
}
function projectProgress(items) {
  const leaves = leafWorkItems(items).filter((item) => !isCancelled(item));
  const completed = leaves.filter(isComplete).length;
  return {total:leaves.length, completed, percent:leaves.length ? Math.round(completed * 100 / leaves.length) : null};
}
function projectHealth(items) {
  const leaves = leafWorkItems(items).filter((item) => !isCancelled(item));
  return {
    total: leaves.length,
    backlog: leaves.filter((item) => ["backlog", "todo"].includes(String(item.state_group || "").toLowerCase())).length,
    active: leaves.filter((item) => ["started", "in_progress"].includes(String(item.state_group || "").toLowerCase())).length,
    completed: leaves.filter(isComplete).length,
    overdue: leaves.filter(isOverdue).length,
    withoutDueDate: leaves.filter((item) => !targetDate(item)).length,
  };
}
...
function map(items, parentFor, project = false) {
  ...
  return `<div class="map-scroll"><div class="strategy-map"><svg class="map-links" aria-hidden="true"></svg><div class="map">${rows.map((row, level) => `<section class="map-row"><p>${level === 0 ? (project ? "Dự án" : "Mục tiêu định hướng") : `Tầng ${level + 1}`}</p><div class="map-cards">${row.map((item) => {
    ...
    const parent = parentFor(item) || "";
    const timing = project && item.remote_id?.startsWith("project-") === false ? `<small class="card-timing ${isOverdue(item) ? "risk" : ""}">${esc(dateText(item))}</small>` : "";
    return `<article class="card ${tone(progress)} ${project && isOverdue(item) ? "risk" : ""}" data-map-node="${esc(id)}" data-map-parent="${esc(parent)}"><div class="card-meta"><i class="dot ${tone(progress)}"></i><small>${esc(project ? "Công việc" : label(kind,item.kind))}</small></div><strong>${esc(item.title)}</strong><span>${esc(project ? label(state,progress) : label(life,progress))}</span>${timing}</article>`;
...
function branch(item, byParent, project = false) {
  ...
  const timing = project && !String(id).startsWith("project-") ? ` · ${dateText(item)}` : "";
  return `<details class="branch ${nested.length ? "" : "leaf"}" ${nested.length ? "open" : ""}><summary><i class="dot ${tone(progress)}"></i><small>${esc(project ? "Công việc" : label(kind,item.kind))}</small><strong>${esc(item.title)}</strong><em class="${project && isOverdue(item) ? "risk" : ""}">${esc(project ? `${label(state,progress)}${timing}` : label(life,progress))}</em></summary>${nested.length ? `<div class="nested">${nested.map((child) => branch(child,byParent,project)).join("")}</div>` : ""}</details>`;
}
...
function mount() {
  ...
  projectContext.id = "project-context";
  projectContext.className = "sub";
  surface.querySelector("#subtitle")?.after(projectContext);
  ...
  const model = {mode:"strategy",view:"dashboard",data:null,agentProjectName:null,requestId:0,strategyId:null,strategies:[],projectFilter:""};
  const currentAgentProject = () => globalThis.Alpine?.store("chats")?.selectedContext?.project?.name || null;
  const strategySelect = document.createElement("select"); strategySelect.title = "Chọn chiến lược"; surface.querySelector("#view")?.after(strategySelect);
  const projectSelect = document.createElement("select"); projectSelect.title = "Lọc dự án"; surface.querySelector("#view")?.after(projectSelect);
  ...
  const draw = () => {
    ...
    if (model.mode === "strategy") {
      ...
      const healthById = new Map((data.project_health?.projects || []).map((item) => [String(item.project_id), item]));
      const rows = objectives.map((objective) => {
        const project = projectsById.get(String(objective.plane_project_ref_id));
        const health = project ? healthById.get(String(project.remote_id)) : null;
        const progress = health ? {total:health.total,percent:health.execution_progress} : {total:0,percent:null};
    ...
    const projectTrees = projects.filter((project) => !model.projectFilter || String(project.remote_id) === model.projectFilter).map((project) => {
      ...
      const health = (data.project_health?.projects || []).find((item) => String(item.project_id) === String(project.remote_id)) || projectHealth(items);
      return `<div class="project-row project-grid"><div><strong>${esc(project.title)}</strong><small>Đồng bộ ${esc(project.synced_at || "chưa có dữ liệu")}</small></div><span><b>${health.backlog + health.todo}</b><small>Cần làm</small></span><span><b>${health.in_progress ?? health.active}</b><small>Đang thực hiện</small></span><span><b>${health.done ?? health.completed}</b><small>Hoàn thành</small></span><span class="${health.overdue ? "overdue" : ""}"><b>${health.overdue}</b><small>Quá hạn</small></span><span><b>${health.without_due_date ?? health.withoutDueDate}</b><small>Chưa đặt hạn</small></span></div>`;
    }).join("");
    const heading = model.view === "map" ? "Bản đồ công việc theo dự án" : model.view === "cascade" ? "Phân cấp công việc" : "Tổng quan theo dự án";
    const overall = (data.project_health?.projects || []).reduce((sum, item) => ({backlog:sum.backlog + item.backlog + item.todo, active:sum.active + item.in_progress, completed:sum.completed + item.done, overdue:sum.overdue + item.overdue}), {backlog:0,active:0,completed:0,overdue:0});
    content.innerHTML = `<div class="kpis"><div class="kpi"><small>Dự án</small><b>${projects.length}</b></div><div class="kpi"><small>Cần làm</small><b>${overall.backlog}</b></div><div class="kpi"><small>Đang thực hiện</small><b>${overall.active}</b></div><div class="kpi"><small>Hoàn thành</small><b>${overall.completed}</b></div><div class="kpi"><small>Quá hạn</small><b>${overall.overdue}</b></div></div><div class="panel"><h2>${heading}</h2>${projectTrees || "<p class='empty'>Chưa có dự án đã đồng bộ.</p>"}</div>`;
    if (model.view === "map") requestAnimationFrame(() => drawMapLinks(content));
  };
  const load = async () => {
    const selectedProject = currentAgentProject(); const requestedStrategy = model.strategyId; const requestId = ++model.requestId;
    content.innerHTML = "<p class='empty'>Đang tải...</p>";
    try { const contextResponse = await callJsonApi("/api/strategy",{action:"get_project_context",agent_project_name:selectedProject}); if (requestId !== model.requestId) return; const context = contextResponse.context || {}; const project = context.agent_project?.name || null; if (!project) { model.data = null; model.agentProjectName = null; projectContext.textContent = "Chưa chọn dự án Agent"; content.innerHTML = "<p class='empty'>Hãy chọn một Dự án Agent để xem dữ liệu.</p>"; return; } model.agentProjectName = project; const strategyResponse = await callJsonApi("/api/strategy",{action:"list_strategies",agent_project_name:project}); if (requestId !== model.requestId) return; model.strategyId = requestedStrategy || context.active_strategy?.id || null; model.strategies = strategyResponse.strategies || []; strategySelect.innerHTML = model.strategies.map((item) => `<option value="${esc(item.id)}">${esc(item.name)} · ${esc(item.status)}</option>`).join("") || `<option value="">Chưa có chiến lược</option>`; strategySelect.value = model.strategyId || ""; projectSelect.innerHTML = `<option value="">Tất cả dự án</option>${(context.plane_projects || []).map((item) => `<option value="${esc(item.remote_id)}">${esc(item.title)}</option>`).join("")}`; const agentLabel = context.agent_project?.legacy_fallback ? "Không gian mặc định (chưa có Agent Project)" : `Dự án Agent: ${project}`; projectContext.textContent = `${agentLabel} · Plane Workspace: ${context.plane_workspace?.plane_workspace_slug || "chưa kết nối"} · Chiến lược: ${model.strategies.find((item) => item.id === model.strategyId)?.name || context.active_strategy?.name || "chưa có"}`; const response = await callJsonApi("/api/strategy",{action:"dashboard",agent_project_name:project,filters:{agent_project_name:project,strategy_id:model.strategyId}}); if (requestId !== model.requestId) return; model.data = response.data; draw(); }
    catch (error) { if (requestId === model.requestId) { console.error(error); content.innerHTML = "<p class='empty'>Không thể tải dữ liệu chiến lược.</p>"; } }
  };
  window.addEventListener("agent-project-changed", () => { model.data = null; if (!surface.hidden) load(); });
  window.addEventListener("darkoffice-context-changed", () => {
    if (!surface.hidden) surface.hidden = true;
  });
  strategySelect.addEventListener("change", () => { model.strategyId = strategySelect.value || null; load(); });
  projectSelect.addEventListener("change", () => { model.projectFilter = projectSelect.value || ""; draw(); });
  ...
  // The surface is a lightweight workspace, not a persistent modal. Core navigation
  ...
  window.addEventListener("hashchange", () => { surface.hidden = true; });
  window.addEventListener("popstate", () => { surface.hidden = true; });
  surface.querySelector("#sync").addEventListener("click",async () => { try { const project = currentAgentProject(); if (!project) throw new Error("Chưa chọn dự án Agent"); await callJsonApi("/api/strategy",{action:"sync",agent_project_name:project}); await load(); frontendNotification({type:"success",message:"Đã yêu cầu đồng bộ Plane cho dự án hiện tại.",frontendOnly:true}); } catch { frontendNotification({type:"error",message:"Không thể đồng bộ Plane cho dự án hiện tại.",frontendOnly:true}); } });
  ...
  surface.querySelector("#strategy-create-form").addEventListener("submit", async (event) => {
    ...
    const node = {agent_project_name:model.agentProjectName,strategy_id:model.strategyId,kind:nodeKind.value,title:surface.querySelector("#strategy-node-title").value.trim(),parent_id:nodeParent.value || null,plane_project_ref_id:nodeProject.value || null,lifecycle:surface.querySelector("#strategy-node-lifecycle").value,description:surface.querySelector("#strategy-node-description").value.trim()};
