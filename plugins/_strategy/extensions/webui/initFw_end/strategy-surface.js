...
function map(items, parentFor, project = false) {
  ...
  return `<div class="map-scroll"><div class="map">${rows.map((row, level) => `<section><p>Tầng ${level + 1}</p>${row.map((item) => {
    const progress = project ? item.state_group : item.lifecycle;
    return `<article class="card ${tone(progress)}"><small>${esc(project ? "Công việc" : label(kind,item.kind))} · ${esc(project ? label(state,progress) : label(life,progress))}</small><strong>${esc(item.title)}</strong></article>`;
...
function mount() {
  ...
  const draw = () => {
    ...
    if (model.mode === "strategy") {
      if (model.view === "map") { content.innerHTML = `<div class="panel"><h2>Bản đồ chiến lược</h2>${map(nodes,(item) => item.parent_id)}</div>`; return; }
