...
function mount() {
  ...
  const draw = () => {
    ...
    if (model.mode === "strategy") {
      ...
      content.innerHTML = `<div class="kpis"><div class="kpi"><small>Mục tiêu</small><b>${objectives.length}</b></div><div class="kpi"><small>Dự án liên kết</small><b>${new Set(rows.map((row) => row.objective.plane_project_ref_id).filter(Boolean)).size}</b></div><div class="kpi"><small>Tiến độ dự án</small><b>${average === null ? "-" : `${average}%`}</b></div></div><div class="panel"><h2>${northStar ? esc(northStar.title) : "Mục tiêu và dự án thực thi"}</h2>${rows.map((row) => `<div class="project objective-grid"><div><strong>${esc(row.objective.title)}</strong><div class="meta">Mục tiêu</div></div><div><strong>${esc(row.project?.title || "Chưa liên kết dự án Plane")}</strong><div class="meta">Dự án thực thi</div></div><b>${row.progress.percent === null ? "Chưa có dữ liệu" : `${row.progress.percent}%`}</b><span>${row.progress.total} công việc</span></div>`).join("") || "<p class='empty'>Chưa có mục tiêu liên kết dự án.</p>"}</div>`; return;
    }
    const projectTrees = projects.map((project) => {
      ...
      return `<div class="project project-grid"><strong>${esc(project.title)}</strong><span>${progress.total} công việc</span><span>${progress.percent === null ? "Chưa có dữ liệu" : `${progress.percent}% hoàn tất`}</span><span>${esc(project.synced_at || "Chưa đồng bộ")}</span></div>`;
    }).join("");
