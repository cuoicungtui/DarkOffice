...
from DarkOffice without keeping a large delegation tool in every agent prompt.

The plugin provides:

- an `orchestrator` skill that tells DarkOffice how to use external CLIs
...
- a Settings > External Services status screen for configured binaries and
  detected auth state;
- adapter metadata for DarkOffice headless, OpenAI Codex CLI, Claude Code,
...
inside the DarkOffice container.
...
| Agent | CLI | Login |
| --- | --- | --- |
| DarkOffice (headless) | `a0` | Instance `/login` session; `A0_USERNAME`/`A0_PASSWORD` in the shell for protected hosts |
...
The `a0` adapter targets the local DarkOffice instance
(`http://localhost:80` inside the container) when no `host`/`AGENT_ZERO_HOST`
is configured, or another DarkOffice instance when you set a host. This lets A0
...
not specify a target, DarkOffice should ask whether to use the same instance or
...
Ask DarkOffice to load/use the `orchestrator` skill before delegating coding
...
If the user does not specify host versus container, DarkOffice asks and saves a
...
to the DarkOffice instance, choose the chat/session or enter the remote/VPS URL,
then press `F4` for Remote Code Execution. Press `F3` too when host file writes
are needed. After that, asking DarkOffice to use local Claude Code/Codex/etc.
...
an externally set `ANTHROPIC_API_KEY`. In DarkOffice, the user can add that
...
If a non-A0 CLI is missing or asks for login/browser confirmation, DarkOffice
...
running the real task. If setup shows provider/menu choices, DarkOffice should
...
```yaml
a0:
  binary: a0          # falls back to /opt/venv/bin/a0 in DarkOffice Docker
