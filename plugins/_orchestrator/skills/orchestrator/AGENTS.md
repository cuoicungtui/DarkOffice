...
- Teach DarkOffice how to delegate work to external terminal/headless coding agents only after the skill is loaded.
...
- Claude Code API-key auth must source DarkOffice secrets from `/a0/usr/.env`, not from the workdir.
- Gemini CLI container runs must use `§§secret(GEMINI_API_KEY)` when that key is in DarkOffice secrets; do not assume it is exported from `/a0/usr/.env`.
...
- State boundaries in imperative language so DarkOffice follows them during chat.
