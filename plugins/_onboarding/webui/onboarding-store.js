...
export const store = createStore("onboarding", {
  ...
  providerMeta(id) {
    ...
    return {
      ...
      short_description: override.short_description || fromDetails.short_description || fallback.short_description || "Connect this provider to DarkOffice.",
    ...
    return LOCAL_PROVIDER_IDS.map((id) => {
      ...
      if (id === "other") {
        return {
          ...
          short_description: "Point DarkOffice at a local compatible server.",
          default_api_base: "",
          api_key_mode: "optional",
        };
      }
  ...
  titleText() {
    ...
    if (this.step === "ready") return "DarkOffice is ready";
  ...
  setupPurpose() {
    if (this.isOAuthProvider()) return "Connect this account, then choose the account-backed model DarkOffice should use.";
    if (this.selectedProviderOrigin === "local") return "Choose a local model and confirm where DarkOffice can reach it.";
    return "Choose a model and add the key DarkOffice will use for this provider.";
  },
