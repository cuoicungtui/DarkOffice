...
Use `code_execution_remote` only for shell-backed execution on the machine where A0 CLI is running. Shells, runtimes, and paths belong to the CLI host, not the DarkOffice server or Docker container.

If the task belongs inside DarkOffice's own runtime, use the normal server-side execution tool instead.
...
- Match the remote host shell syntax. A Windows CLI may need PowerShell syntax even when DarkOffice runs on Linux.
