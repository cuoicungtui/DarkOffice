...
def build_context(context_id: str = "", max_items: int = 20) -> str:
    ...
    lines = [
        "These Markdown or plain text files are open in the Editor for the active DarkOffice context. Content is omitted; use `text_editor` with action `read` before content-sensitive edits.",
    ]
    for item in files:
