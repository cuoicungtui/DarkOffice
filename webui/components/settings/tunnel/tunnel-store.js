...
const model = {
  ...
  get loginActionCopy() {
    if (this.loginProvider === "tailscale") {
      return "Open the Tailscale link to approve this container or enable Funnel. DarkOffice will continue when Tailscale reports the public URL.";
    }
    return "Approve the tunnel request, then DarkOffice will finish enabling Remote Control.";
  },
      ...
      if (!hasAuth) {
        const proceed = confirm(
          "Remote Control works best with sign-in enabled.\n\n" +
            "Without a login, anyone with the URL can reach this DarkOffice instance.\n\n" +
